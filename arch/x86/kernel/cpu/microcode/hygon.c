...
static int get_patch_level_cpu(unsigned int cpu, u32 *rev)
{
        u64 val;
        int ret = 0;
        ...
        if (cpu == smp_processor_id()) {
                ...
                ret = rdmsrq_safe_on_cpu(cpu, HYGON_MSR_PATCH_LEVEL, &val);
                if (!ret)
                        *rev = (u32)val;
        }
				  ...
				  unsigned int patch_size)
{
	...
	if (IS_ENABLED(CONFIG_MICROCODE_DBG) && x86_hypervisor_present)
		microcode_rev[cpu] = mc->header.patch_level;

	if (get_patch_level_cpu(cpu, cur_rev) != 0) {
                pr_err("Failed to read microcode revision for CPU%d\n", cpu);
                return false;
        }
...
void __init load_ucode_hygon_bsp(struct early_load_data *ed, unsigned int cpuid_1_eax)
{
	...
	if (get_patch_level_cpu(0, &rev) != 0) {
		pr_err("Failed to read BSP microcode revision\n");
		return;
	}
	ed->old_rev = rev;
...
static struct hygon_ucode_patch *find_hygon_patch(unsigned int cpu)
{
	...
	u32 rev;
        ...
        if (get_patch_level_cpu(cpu, &rev) != 0) {
                pr_err("Failed to read microcode revision for CPU%d\n", cpu);
                return NULL;
        }
