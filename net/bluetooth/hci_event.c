						...
						struct sk_buff *skb)
{
	...
	if (!status) {
		conn->link_supervision_timeout_ms = __le16_to_cpu(sent->timeout_ms);
		bt_dev_dbg(hdev, "handle 0x%4.4x timeout set to 0x%4.4x (%u ms)",
			   handle, conn->link_supervision_timeout_ms,
			   conn->link_supervision_timeout_ms * 5 / 8);
	}
				  ...
				  struct sk_buff *skb)
{
	...
	if (!status) {
		...
		if (conn->type == ACL_LINK && conn->role == HCI_ROLE_MASTER) {
			...
			cp.handle = ev->handle;
			cp.timeout_ms = 0x1F40;	/* 8000 * 0.625ms = 5000ms */
