...
insert:
...
#ifdef IP_SET_HASH_WITH_MULTI
	if (old)
		ret = rhashtable_replace_fast(&h->rhlt.ht, &old->node.rhead,
					      &e->node.rhead, mtype_rht_params);
	else
	...
	if (ret) {
		...
		/* Update the new elements counters with the replaced
		 * one, unless userspace gave specific start values.
		 *
		 * old_c is still visible to other CPUs, so this is
		 * best-effort only.
		 */
