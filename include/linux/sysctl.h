 ...
 *
 * Only lvalue-ness is checked; any addressable type is accepted.
 *
 ...
 *
 * exact function-type match via sizeof trick.
 * including wrong parameter types or qualifiers.
 * @__handler: proc_handler
 */
#define __CTL_HANDLER(__handler) \
	(0 ?								\
		((void)sizeof(char[__builtin_types_compatible_p(	\
			__typeof__(__handler), proc_handler) ?		\
			1 : -1]), NULL) :				\
	__handler)
