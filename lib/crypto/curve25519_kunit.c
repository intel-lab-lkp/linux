 ...
 * Author: João Pedro Leôncio <jpleoncio@lkcamp.dev>
...
static void curve25519_test(struct kunit *test)
{
	...
	for (i = 0; i < ARRAY_SIZE(curve25519_test_vectors); i++) {
		...
		KUNIT_EXPECT_EQ(test, ret, curve25519_test_vectors[i].valid);
		KUNIT_EXPECT_EQ(test, memcmp(out, curve25519_test_vectors[i].result,
...
static void curve25519_generate_public_test(struct kunit *test)
{
	...
	u8 public[CURVE25519_KEY_SIZE] = { CURVE25519_MAGIC_NUMBER };

	for (i = 0; i < ARRAY_SIZE(curve25519_test_vectors); i++) {
		memset(out, 0, CURVE25519_KEY_SIZE);
		ret = curve25519_generate_public(out, curve25519_test_vectors[i].private);
		ret2 = curve25519(out2, curve25519_test_vectors[i].private, public);
		KUNIT_EXPECT_EQ(test, ret, ret2);
		KUNIT_EXPECT_EQ(test, memcmp(out, out2, CURVE25519_KEY_SIZE), 0);
	}
}
...
static void curve25519_generic_test(struct kunit *test)
{
	...
	for (i = 0; i < ARRAY_SIZE(curve25519_test_vectors); i++) {
		memset(out, 0, CURVE25519_KEY_SIZE);
		curve25519_generic(out, curve25519_test_vectors[i].private,
				   curve25519_test_vectors[i].public);
		KUNIT_EXPECT_EQ(test, memcmp(out, curve25519_test_vectors[i].result,
			   CURVE25519_KEY_SIZE), 0);
	}
...
static struct kunit_case curve25519_test_cases[] = {
	KUNIT_CASE(curve25519_test),
	KUNIT_CASE(curve25519_generate_public_test),
	KUNIT_CASE(curve25519_generic_test),
	{},
};
...
MODULE_AUTHOR("João Pedro Leôncio <jpleoncio@lkcamp.dev>");
