...
/*
 * Run one op-table.  A plain coef write (strobe == 0) uses the normal
 * codec coef registers via alc_write_coef_idx().  A mailbox op selects
 * a bank, then streams four PROC_COEF words (addr/hi/lo/commit) into a
 * single coef index (0x23) on node 0x20 to reach the external DSP --
 * that one-index/many-words form is why the mailbox cannot be written
 * as an alc_process_coef_fw() table (which is one value per index).
 */
