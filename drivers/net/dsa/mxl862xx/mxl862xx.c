...
static void mxl862xx_remove(struct mdio_device *mdiodev)
{
	...
	/* Cancel the stats poll only after the user netdevs are gone: until
	 * dsa_unregister_switch() returns a get_stats64() can still re-arm the
	 * work after a plain cancel, and it would then run against the
	 * devres-freed priv.
	 */
	cancel_delayed_work_sync(&priv->stats_work);
