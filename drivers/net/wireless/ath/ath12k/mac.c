...
static struct ieee80211_bss_conf *ath12k_get_link_bss_conf(struct ath12k_link_vif *arvif)
{
...
ieee80211_link_sta *ath12k_get_link_sta(struct ath12k_link_sta *arsta)
{
	...
	/* ieee80211_sta->link[]'s are rcu objects which requires rcu_read_lock()
	 * to be held for safe access.
	 */
				   ...
				   enum ieee80211_sta_state new_state)
{
	...
	if (old_state == IEEE80211_STA_NOTEXIST &&
		...
		if (sta->mlo && !ahsta->links_map &&
			...
			ret = ath12k_peer_mlo_create(ah, sta);
			if (ret) {
					...
					u32 changed)
{
	...
	/* KVALO: ath12k_get_ar_by_vif() requires wiphy_lock() to be held
	 * but we cannot take it as we are atomic here
	 */
				 ...
				 struct ath12k_vif *ahvif, u8 link_id)
{
	...
	rcu_read_lock();
	if (WARN_ON(rcu_dereference(ahsta->link[link_id]))) {
		rcu_read_unlock();
		return NULL;
	}
	rcu_read_unlock();
					  ...
					  u16 old_links, u16 new_links)
{
	...
	/* Firmware does not support removal of one of link stas.
	 * All sta would be removed during ML STA
...
static void ath12k_mgmt_over_wmi_tx_work(struct wiphy *wiphy, struct wiphy_work *work)
{
	...
	while ((skb = skb_dequeue(&ar->wmi_mgmt_tx_queue)) != NULL) {
		...
		if (ar->allocated_vdev_map & (1LL << arvif->vdev_id)) {
			...
			ath12k_warn(ar->ab,
				    "dropping mgmt frame for vdev %d, is_started %d link_id %u\n",
				    arvif->vdev_id,
				    arvif->is_started,
				    skb_cb->link_id);
			ath12k_mgmt_over_wmi_tx_drop(ar, skb);
				 ...
				 u8 link, struct sk_buff *skb, u32 info_flags)
{
	...
	if (ieee80211_is_mgmt(hdr->frame_control)) {
		...
		if (bss_conf && link_sta) {
			 ...
			 * likns on which the ML AP is active, but now sends some frame
