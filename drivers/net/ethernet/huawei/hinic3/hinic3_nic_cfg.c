...
static bool hinic3_check_pf_set_vf_already(struct hinic3_hwdev *hwdev,
					   u8 status)
{
				 ...
				 u16 vlan_id)
{
	if ((status && status != MGMT_STATUS_EXIST) ||
		...
		if (hinic3_check_pf_set_vf_already(hwdev, status))
			return 0;
		   ...
		   u16 func_id)
{
	...
	if (hinic3_check_pf_set_vf_already(hwdev, mac_info.msg_head.status)) {
		   ...
		   u16 func_id)
{
	...
	if (err || (mac_info.msg_head.status &&
		    !hinic3_check_pf_set_vf_already(hwdev, mac_info.msg_head.status))) {
		dev_err(hwdev->dev,
