				    ...
				    u32 *indir_table)
{
	...
	memset(pair.out->buf, 0, le16_to_cpu(pair.out->size));
	...
	/* Note: The firmware internally handles the byte order for the GET
	 * response. Unlike the SET path, no swab32 conversion is needed here.
	 */
	for (i = 0; i < L2NIC_RSS_INDIR_SIZE; i++)
...
static int hinic3_set_hw_hash_type(struct net_device *netdev,
...
int hinic3_set_rxfh(struct net_device *netdev, struct ethtool_rxfh_param *rxfh,
		    struct netlink_ext_ack *extack)
{
	enum hinic3_rss_hash_type old_hash_type, new_hash_type;
	struct hinic3_nic_dev *nic_dev = netdev_priv(netdev);
	int err;
	...
	old_hash_type = nic_dev->rss_hash_type;
	new_hash_type = old_hash_type;

	switch (rxfh->hfunc) {
	case ETH_RSS_HASH_NO_CHANGE:
		break;
	case ETH_RSS_HASH_XOR:
		new_hash_type = HINIC3_RSS_HASH_ENGINE_TYPE_XOR;
		break;
	case ETH_RSS_HASH_TOP:
		new_hash_type = HINIC3_RSS_HASH_ENGINE_TYPE_TOEP;
		break;
	default:
	...
	err = hinic3_set_hw_hash_type(netdev, new_hash_type);
	if (err)
		return err;

	err = hinic3_set_rss_rxfh(netdev, rxfh->indir, rxfh->key);
	if (err)
		hinic3_set_hw_hash_type(netdev, old_hash_type);
