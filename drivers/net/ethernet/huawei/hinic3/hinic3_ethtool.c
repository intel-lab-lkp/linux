	...
	if (old_adaptive) {
		...
		if (is_up) {
			for (i = 0; i < nic_dev->q_params.num_qps; i++) {
				...
				rxq->dim.profile_ix = 0;
				rxq->dim.tune_state = 0;
				rxq->dim.steps_right = 0;
				rxq->dim.steps_left = 0;
				rxq->dim.tired = 0;
				memset(&rxq->dim.prev_stats, 0,
				       sizeof(rxq->dim.prev_stats));
			}
