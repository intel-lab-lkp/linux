 ...
 *
 * Pin masks, shifts, and counts are now defined in <linux/mfd/mc33978.h>
 * to avoid duplication across drivers.
 */
...
{
	return pin >= MC33978_NUM_SG_PINS;
}
...
{
	return mc33978_is_sp(pin) ? pin - MC33978_NUM_SG_PINS : pin;
}
...
{
	return mc33978_is_sp(pin) ? 3 * (pin - MC33978_NUM_SG_PINS) : 3 * (pin % 8);
}
			     ...
			     enum mc33978_drive_mode mode)
{
	...
	if (!ret) {
		...
		if (mpc->cached_bias[pin] != MC33978_TRISTATE)
			mpc->cached_bias[pin] = (mode == MC33978_OPEN_DRAIN) ?
						MC33978_PD : MC33978_PU;
	}
			    ...
			    enum mc33978_bias_state val)
{
	...
	if (mc33978_is_sp(pin) && val != MC33978_TRISTATE) {
		...
		mpc->cached_drive[pin] = (val == MC33978_PD) ?
					 MC33978_OPEN_DRAIN : MC33978_OPEN_SOURCE;
	}
				 ...
				 unsigned long mask, unsigned long *state)
{
	...
	if (mask & MC33978_SP_PIN_MASK) {
		ret = mc33978_read(mpc, MC33978_REG_CONFIG, &config_reg);
		if (ret)
			return ret;
	}

	*state = mc33978_raw_to_physical_state(status, config_reg) & mask;
...
static int mc33978_update_tri_state(struct mc33978_pinctrl *mpc, u32 mask,
				    u32 bits)
{
	u32 sgmask = (mask & MC33978_SG_PIN_MASK) >> MC33978_SG_SHIFT;
	u32 sgbits = (bits & MC33978_SG_PIN_MASK) >> MC33978_SG_SHIFT;
	u32 spmask = (mask & MC33978_SP_PIN_MASK) >> MC33978_SP_SHIFT;
	u32 spbits = (bits & MC33978_SP_PIN_MASK) >> MC33978_SP_SHIFT;
	u32 config_reg = 0;
	int ret = 0;
...
static void mc33978_gpio_irq_enable(struct irq_data *data)
{
	...
	else if (parent && parent->chip && parent->chip->irq_unmask)
		parent->chip->irq_unmask(parent);
}

static void mc33978_gpio_irq_disable(struct irq_data *data)
{
	struct gpio_chip *gc = irq_data_get_irq_chip_data(data);
	struct irq_data *parent = data->parent_data;

	if (parent && parent->chip && parent->chip->irq_disable)
		parent->chip->irq_disable(parent);
	else if (parent && parent->chip && parent->chip->irq_mask)
		parent->chip->irq_mask(parent);
...
static const struct irq_chip mc33978_gpio_irqchip = {
	...
	.irq_enable = mc33978_gpio_irq_enable,
	.irq_disable = mc33978_gpio_irq_disable,
...
static int mc33978_add_pin_ranges(struct gpio_chip *chip)
{
	return gpiochip_add_pin_range(chip, dev_name(chip->parent),
				      0, 0, MC33978_NGPIO);
}
				   ...
				   struct device *dev)
{
	...
	mpc->chip.add_pin_ranges = mc33978_add_pin_ranges;
