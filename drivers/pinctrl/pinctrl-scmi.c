...
{
	unsigned int nr_funcs, nr_groups, i, j;
	...
	for (i = 0; i < nr_funcs; i++) {
		...
		ret = pinmux_generic_add_function(pmx->pctldev, fname, gnames,
				  nr_groups, NULL);
		if (ret < 0)
			return ret;
		if (ret != i) {
			dev_err(pmx->dev,
				"Duplicate function name: %s index: %u selector: %d\n",
				fname, i, ret);
			return -EINVAL;
		}
	}
