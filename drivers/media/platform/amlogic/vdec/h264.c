...
static inline bool frame_size_in_range(const struct v4l2_frmsize_stepwise *stepwise,
...
static int config_sps_params(struct aml_h264_ctx *h264_ctx,
			     ...
			     const struct v4l2_ctrl_h264_sps *sps)
{
	...
	if (!frame_size_in_range(stepwise, h264_ctx->frame_width,
		...
		dev_info(&ctx->dev->plat_dev->dev,
			 "invalid SPS dimensions %ux%u, supported range %ux%u-%ux%u\n",
			 h264_ctx->frame_width, h264_ctx->frame_height,
...
static int h264_config_params(struct aml_vdec_ctx *ctx)
{
	...
	ret = config_sps_params(h264_ctx, p_sps_base, sps);
	if (ret)
		return ret;
				 ...
				 u32 mb_width, u32 mb_height)
{
	...
	if (ret) {
		dev_err(hw->dev, "failed to alloc u canvas (%d)\n", ret);
		meson_canvas_free(hw->canvas, buf_spec->y_canvas_index);
		return;
	}
	...
	if (buf_spec->canvas_pos < 0) {
		dev_err(hw->dev, "invalid canvas_pos, skip ANC canvas config\n");
		return;
	}
			      ...
			      struct aml_vdec_ctx *ctx)
{
	...
	h264_ctx->colocated_buf_num = min_t(u32,
					    h264_ctx->max_num_ref_frames + COL_BUFFER_MARGIN,
					    H264_MAX_COL_BUF);
...
static int get_ref_list_size(struct aml_h264_ctx *h264_ctx, int cur_list)
{
	...
	if (cur_list == 0) {
		...
		if (override_flag)
			num_ref_idx_lx_active_minus1 =
			    h264_ctx->dpb_param.dpb.num_ref_idx_l1_active_minus1;
	}
			       ...
			       int pic_num_lx, int *ref_idx_lx)
{
	...
	if (num_ref_idx_lx_active > V4L2_H264_NUM_DPB_ENTRIES) {
			      ...
			      int lt_pic_num, int *ref_idx_lx)
{
	...
	num_ref_idx_lx_active = get_ref_list_size(h264_ctx, cur_list);
	if (num_ref_idx_lx_active > V4L2_H264_NUM_DPB_ENTRIES) {
		dev_dbg(&ctx->dev->plat_dev->dev, "incorrect value lt num_ref_idx_lx_active %d\n",
			num_ref_idx_lx_active);
		return;
	}
...
static int h264_config_decode_spec(struct aml_vdec_hw *hw, struct aml_vdec_ctx *ctx)
{
...
static irqreturn_t h264_threaded_isr_func(int irq, void *priv)
{
	...
	case H264_SLICE_HEADER_DONE:
		...
		if (h264_config_decode_spec(hw, ctx) < 0) {
			ctx->int_cond = 1;
