			       ...
			       struct v4l2_subdev_state *sd_state)
{
	...
	if (!csi->is_csi2) {
		...
		sink_fmt = v4l2_subdev_get_format(sd_state, IMX7_CSI_PAD_SINK,
						  0);
			     ...
			     struct v4l2_subdev_state *sd_state)
{
	...
	for (i = 0; i < IMX7_CSI_PADS_NUM; i++) {
		struct v4l2_mbus_framefmt *mf =
			v4l2_subdev_get_format(sd_state, i, 0);
				   ...
				   struct v4l2_subdev_mbus_code_enum *code)
{
	...
	in_fmt = v4l2_subdev_get_format(sd_state, IMX7_CSI_PAD_SINK, 0);
			    ...
			    struct v4l2_subdev_format *sdformat)
{
	...
	fmt = v4l2_subdev_get_format(sd_state, sdformat->pad, 0);
	...
	*fmt = sdformat->format;
	...
	if (sdformat->pad == IMX7_CSI_PAD_SINK) {
		...
		outfmt = v4l2_subdev_get_format(sd_state, IMX7_CSI_PAD_SRC, 0);
