...
static int mipi_csis_s_stream(struct v4l2_subdev *sd, int enable)
{
	...
	format = v4l2_subdev_get_format(state, CSIS_PAD_SINK, 0);
				    ...
				    struct v4l2_subdev_mbus_code_enum *code)
{
	...
	if (code->pad == CSIS_PAD_SOURCE) {
		...
		fmt = v4l2_subdev_get_format(sd_state, code->pad, 0);
		code->code = fmt->code;
		return 0;
	}
			     ...
			     struct v4l2_subdev_format *sdformat)
{
	...
	fmt = v4l2_subdev_get_format(sd_state, CSIS_PAD_SOURCE, 0);
	*fmt = sdformat->format;
				    ...
				    struct v4l2_mbus_frame_desc *fd)
{
	...
	state = v4l2_subdev_lock_and_get_active_state(sd);
	fmt = v4l2_subdev_get_format(state, CSIS_PAD_SOURCE, 0);
