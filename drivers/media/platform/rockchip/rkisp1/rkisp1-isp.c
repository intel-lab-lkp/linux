			      ...
			      struct v4l2_subdev_state *sd_state)
{
	const struct v4l2_rect *src_crop =
		v4l2_subdev_get_crop(sd_state, RKISP1_ISP_PAD_SOURCE_VIDEO, 0);
			     ...
			     enum v4l2_mbus_type mbus_type, u32 mbus_flags)
{
	...
	sink_frm = v4l2_subdev_get_format(sd_state, RKISP1_ISP_PAD_SINK_VIDEO,
					  0);
	sink_crop = v4l2_subdev_get_crop(sd_state, RKISP1_ISP_PAD_SINK_VIDEO,
					 0);
	src_frm = v4l2_subdev_get_format(sd_state,
					 RKISP1_ISP_PAD_SOURCE_VIDEO, 0);
			     ...
			     struct v4l2_subdev_state *sd_state)
{
	...
	src_fmt = v4l2_subdev_get_format(sd_state,
				         RKISP1_ISP_PAD_SOURCE_VIDEO, 0);
				  ...
				  struct v4l2_subdev_state *sd_state)
{
	...
	/* Video. */
	sink_fmt = v4l2_subdev_get_format(sd_state, RKISP1_ISP_PAD_SINK_VIDEO,
					  0);
	...
	*src_fmt = *sink_fmt;
	...
	src_crop = v4l2_subdev_get_crop(sd_state, RKISP1_ISP_PAD_SOURCE_VIDEO,
					0);
	*src_crop = *sink_crop;

	/* Parameters and statistics. */
	sink_fmt = v4l2_subdev_get_format(sd_state,
					  RKISP1_ISP_PAD_SINK_PARAMS, 0);
	src_fmt = v4l2_subdev_get_format(sd_state,
					 RKISP1_ISP_PAD_SOURCE_STATS, 0);
	sink_fmt->width = 0;
	sink_fmt->height = 0;
	sink_fmt->field = V4L2_FIELD_NONE;
			      ...
			      struct v4l2_subdev_format *fmt)
{
	...
	if (fmt->pad == RKISP1_ISP_PAD_SINK_VIDEO)
		...
		fmt->format = *v4l2_subdev_get_format(sd_state, fmt->pad, 0);
				    ...
				    struct v4l2_subdev_selection *sel)
{
	...
	case V4L2_SEL_TGT_CROP_BOUNDS:
		if (sel->pad == RKISP1_ISP_PAD_SINK_VIDEO) {
			...
			fmt = v4l2_subdev_get_format(sd_state, sel->pad, 0);
			sel->r.height = fmt->height;
			sel->r.width = fmt->width;
			sel->r.left = 0;
			sel->r.top = 0;
		} else {
			sel->r = *v4l2_subdev_get_crop(sd_state,
