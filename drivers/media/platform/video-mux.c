				...
				const struct media_pad *remote, u32 flags)
{
	...
	if (flags & MEDIA_LNK_FL_ENABLED) {
		...
		source_mbusformat = v4l2_subdev_get_format(sd_state,
							   source_pad, 0);
		*source_mbusformat = *v4l2_subdev_get_format(sd_state,
							     vmux->active, 0);
			    ...
			    struct v4l2_subdev_format *sdformat)
{
	...
	mbusformat = v4l2_subdev_get_format(sd_state, sdformat->pad, 0);
	if (!mbusformat)
		return -EINVAL;

	source_mbusformat = v4l2_subdev_get_format(sd_state, source_pad, 0);
	...
	if ((pad->flags & MEDIA_PAD_FL_SOURCE) && vmux->active >= 0)
		sdformat->format = *v4l2_subdev_get_format(sd_state,
							   vmux->active, 0);
			      ...
			      struct v4l2_subdev_state *sd_state)
{
	...
	for (i = 0; i < sd->entity.num_pads; i++) {
		mbusformat = v4l2_subdev_get_format(sd_state, i, 0);
