...
static int imx296_setup(struct imx296 *sensor, struct v4l2_subdev_state *state)
{
	...
	format = v4l2_subdev_get_format(state, 0, 0);
	crop = v4l2_subdev_get_crop(state, 0, 0);
			     ...
			     struct v4l2_subdev_format *fmt)
{
	...
	crop = v4l2_subdev_get_crop(state, fmt->pad, 0);
				...
				struct v4l2_subdev_selection *sel)
{
	...
	case V4L2_SEL_TGT_CROP:
		sel->r = *v4l2_subdev_get_crop(state, sel->pad, 0);
		break;
				...
				struct v4l2_subdev_selection *sel)
{
	...
	crop = v4l2_subdev_get_crop(state, sel->pad, 0);
	...
	if (rect.width != crop->width || rect.height != crop->height) {
		 ...
		 * been modified.
		 */
		format = v4l2_subdev_get_format(state, sel->pad, 0);
