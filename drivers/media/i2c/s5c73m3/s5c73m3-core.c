				   ...
				   const struct s5c73m3_frame_size **fs)
{
	...
	default:
		...
		if (fmt->which == V4L2_SUBDEV_FORMAT_ACTIVE)
			...
			*fs = s5c73m3_find_frame_size(v4l2_subdev_get_format(sd_state, OIF_ISP_PAD, 0),
						      RES_ISP);
		break;
	}
	...
	if (fmt->which == V4L2_SUBDEV_FORMAT_TRY) {
		...
		*mf = fmt->format;
		if (fmt->pad == OIF_ISP_PAD) {
			mf = v4l2_subdev_get_format(sd_state, OIF_SOURCE_PAD,
						    0);
	...
	if (fse->pad == OIF_SOURCE_PAD) {
		...
		case S5C73M3_ISP_FMT: {
			...
			if (fse->which == V4L2_SUBDEV_FORMAT_TRY) {
				...
				mf = v4l2_subdev_get_format(sd_state,
							    OIF_ISP_PAD, 0);
...
static int s5c73m3_open(struct v4l2_subdev *sd, struct v4l2_subdev_fh *fh)
{
	...
	mf = v4l2_subdev_get_format(fh->state, S5C73M3_ISP_PAD, 0);
	s5c73m3_fill_mbus_fmt(mf, &s5c73m3_isp_resolutions[1],
						S5C73M3_ISP_FMT);

	mf = v4l2_subdev_get_format(fh->state, S5C73M3_JPEG_PAD, 0);
...
static int s5c73m3_oif_open(struct v4l2_subdev *sd, struct v4l2_subdev_fh *fh)
{
	...
	mf = v4l2_subdev_get_format(fh->state, OIF_ISP_PAD, 0);
	s5c73m3_fill_mbus_fmt(mf, &s5c73m3_isp_resolutions[1],
						S5C73M3_ISP_FMT);

	mf = v4l2_subdev_get_format(fh->state, OIF_JPEG_PAD, 0);
	s5c73m3_fill_mbus_fmt(mf, &s5c73m3_jpeg_resolutions[1],
					S5C73M3_JPEG_FMT);

	mf = v4l2_subdev_get_format(fh->state, OIF_SOURCE_PAD, 0);
	s5c73m3_fill_mbus_fmt(mf, &s5c73m3_isp_resolutions[1],
						S5C73M3_ISP_FMT);
	return 0;
}
