			      ...
			      struct v4l2_subdev_format *format)
{
	...
	switch (format->which) {
	case V4L2_SUBDEV_FORMAT_TRY:
		sensor_format = v4l2_subdev_get_format(sd_state, format->pad,
						       0);
		break;
	default:
			      ...
			      struct v4l2_subdev_format *format)
{
	...
	if (format->which == V4L2_SUBDEV_FORMAT_TRY) {
		*v4l2_subdev_get_format(sd_state, format->pad, 0) = mode->format;
	} else {
...
static int ov5647_open(struct v4l2_subdev *sd, struct v4l2_subdev_fh *fh)
{
	...
	struct v4l2_rect *crop = v4l2_subdev_get_crop(fh->state, 0, 0);
