			     ...
			     bool needs_scramb)
{
	...
	supp_scramb = dw_hdmi_qp_supports_scrambling(hdmi,
						     &connector->display_info);
	dev_dbg(hdmi->dev, "%s HDMI mode, supp_scramb=%d, need_scramb=%d\n",
		__func__, supp_scramb, needs_scramb);
	...
	if (supp_scramb) {
		...
		drm_scdc_set_high_tmds_clock_ratio(connector, needs_scramb);
		drm_scdc_set_scrambling(connector, needs_scramb);
	}

	dw_hdmi_qp_write(hdmi, supp_scramb && needs_scramb, SCRAMB_CONFIG0);
}
					    ...
					    struct drm_bridge_state *old_state)
{
	...
	dw_hdmi_qp_setup(hdmi, connector,
			 conn_state->hdmi.tmds_char_rate > HDMI14_MAX_TMDSCLK);
