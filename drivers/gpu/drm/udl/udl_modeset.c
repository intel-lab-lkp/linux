...
static void udl_crtc_helper_atomic_enable(struct drm_crtc *crtc, struct drm_atomic_update *state)
{
...
static void udl_crtc_helper_atomic_disable(struct drm_crtc *crtc, struct drm_atomic_update *state)
{
	struct drm_device *dev = crtc->dev;
	struct udl_device *udl = to_udl(dev);
	struct urb *urb;
