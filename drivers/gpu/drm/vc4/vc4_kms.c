...
static struct vc4_ctm_state *vc4_get_ctm_state(struct drm_atomic_update *state,
...
static void
vc4_ctm_commit(struct vc4_dev *vc4, struct drm_atomic_update *state)
{
...
vc4_hvs_get_new_global_state(const struct drm_atomic_update *state)
{
...
struct vc4_hvs_state *
vc4_hvs_get_old_global_state(const struct drm_atomic_update *state)
{
	struct vc4_dev *vc4 = to_vc4_dev(state->dev);
	struct drm_private_state *priv_state;
...
struct vc4_hvs_state *
vc4_hvs_get_global_state(struct drm_atomic_update *state)
{
	struct vc4_dev *vc4 = to_vc4_dev(state->dev);
	struct drm_private_state *priv_state;
...
static void vc4_atomic_commit_tail(struct drm_atomic_update *state)
{
	struct drm_device *dev = state->dev;
	struct vc4_dev *vc4 = to_vc4_dev(dev);
	struct vc4_hvs *hvs = vc4->hvs;
...
static int vc4_atomic_commit_setup(struct drm_atomic_update *state)
{
	struct drm_crtc_state *crtc_state;
 ...
 */
static int
vc4_ctm_atomic_check(struct drm_device *dev, struct drm_atomic_update *state)
{
	struct vc4_dev *vc4 = to_vc4_dev(dev);
...
static int vc4_load_tracker_atomic_check(struct drm_atomic_update *state)
{
...
static int
vc4_core_clock_atomic_check(struct drm_atomic_update *state)
{
	struct vc4_dev *vc4 = to_vc4_dev(state->dev);
	struct drm_private_state *priv_state;
	struct vc4_hvs_state *hvs_new_state;
...
static int
vc4_atomic_check(struct drm_device *dev, struct drm_atomic_update *state)
{
	int ret;
