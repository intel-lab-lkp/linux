...
static bool can_do_async(struct drm_atomic_update *state,
 ...
 */
static unsigned get_crtc_mask(struct drm_atomic_update *state)
{
	struct drm_crtc_state *crtc_state;
	struct drm_crtc *crtc;
	unsigned i, mask = 0;
...
int msm_atomic_check(struct drm_device *dev, struct drm_atomic_update *state)
{
...
void msm_atomic_commit_tail(struct drm_atomic_update *state)
{
