...
int mgag200_crtc_helper_atomic_check(struct drm_crtc *crtc, struct drm_atomic_update *new_state)
{
	struct drm_device *dev = crtc->dev;
	struct mga_device *mdev = to_mga_device(dev);
...
void mgag200_crtc_helper_atomic_flush(struct drm_crtc *crtc, struct drm_atomic_update *old_state)
{
...
void mgag200_crtc_helper_atomic_enable(struct drm_crtc *crtc, struct drm_atomic_update *old_state)
{
	struct drm_device *dev = crtc->dev;
	struct mga_device *mdev = to_mga_device(dev);
	const struct mgag200_device_funcs *funcs = mdev->funcs;
...
void mgag200_crtc_helper_atomic_disable(struct drm_crtc *crtc, struct drm_atomic_update *old_state)
{
	struct mga_device *mdev = to_mga_device(crtc->dev);
...
static void mgag200_mode_config_helper_atomic_commit_tail(struct drm_atomic_update *state)
{
