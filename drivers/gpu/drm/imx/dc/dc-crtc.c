...
static int
dc_crtc_atomic_check(struct drm_crtc *crtc, struct drm_atomic_update *state)
{
...
static void
dc_crtc_atomic_begin(struct drm_crtc *crtc, struct drm_atomic_update *state)
{
	struct drm_crtc_state *new_crtc_state =
				drm_atomic_get_new_crtc_state(state, crtc);
...
static void
dc_crtc_atomic_flush(struct drm_crtc *crtc, struct drm_atomic_update *state)
{
...
static void
dc_crtc_atomic_enable(struct drm_crtc *crtc, struct drm_atomic_update *state)
{
	struct drm_crtc_state *new_crtc_state =
				drm_atomic_get_new_crtc_state(state, crtc);
	struct drm_display_mode *adj = &new_crtc_state->adjusted_mode;
...
static void
dc_crtc_atomic_disable(struct drm_crtc *crtc, struct drm_atomic_update *state)
{
	struct drm_crtc_state *new_crtc_state =
				drm_atomic_get_new_crtc_state(state, crtc);
	struct dc_drm_device *dc_drm = to_dc_drm_device(crtc->dev);
