...
static void kunit_action_drm_atomic_update_put(void *ptr)
{
	struct drm_atomic_update *state = ptr;
 ...
 * drm_atomic_update_put() on it, it will be done so automatically.
 *
 * Returns:
				    ...
				    struct drm_modeset_acquire_ctx *ctx)
{
	...
	state = drm_atomic_update_alloc(drm);
	if (!state)
		return ERR_PTR(-ENOMEM);

	ret = kunit_add_action_or_reset(test,
					kunit_action_drm_atomic_update_put,
					state);
	if (ret)
		return ERR_PTR(ret);
