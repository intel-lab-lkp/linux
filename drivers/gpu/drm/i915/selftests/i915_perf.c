...
static int live_noa_delay(void *arg)
{
	...
	drm_info(&i915->drm,
		 "CPU delay: %lluns, expected %lluns\n", ktime_sub(t1, t0),
		 expected);
...
static int live_noa_gpr(void *arg)
{
	...
	*cs++ = MI_NOOP;
	...
	if (i915_request_wait(rq, I915_WAIT_INTERRUPTIBLE, HZ / 2) < 0) {
		drm_err(&i915->drm, "noa_wait timed out\n");
	...
	for (i = 0; i < 32; i++) {
		...
		drm_err(&i915->drm,
			"GPR[%d] lost, found:%08x, expected:%08x!\n", i,
			store[i], STACK_MAGIC);
		err = -EINVAL;
	}
...
int i915_perf_live_selftests(struct drm_i915_private *i915)
{
	...
	err = i915_live_subtests(tests, i915, &i915->drm);
