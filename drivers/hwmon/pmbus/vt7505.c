...
{
	u32 ocp_severe_filter;
	int ret;
	u16 word;

	if (of_property_read_u32(client->dev.of_node, "adi,ocp-severe-filter",
				 &ocp_severe_filter))
		return 0;

	if (!FIELD_FIT(VT7505_CFG_OCP_S_FILT_MASK, ocp_severe_filter))
		return dev_err_probe(&client->dev, -EINVAL,
				     "invalid adi,ocp-severe-filter value %u\n",
				     ocp_severe_filter);
	...
	word |= FIELD_PREP(VT7505_CFG_OCP_S_FILT_MASK, ocp_severe_filter);
