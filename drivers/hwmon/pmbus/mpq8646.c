...
/* Shared debugfs root, created in module_init and removed in module_exit. */
static struct dentry *mpq8646_debugfs_root;
...
static void mpq8646_debugfs_register(struct mpq8646_priv *priv)
{
	...
	root = debugfs_create_dir(name, mpq8646_debugfs_root);
...
static void mpq8646_debugfs_root_create(void)
{
	mpq8646_debugfs_root = debugfs_create_dir(MPQ8646_DEBUGFS_ROOT_NAME, NULL);
}

static void mpq8646_debugfs_root_destroy(void)
{
	debugfs_remove_recursive(mpq8646_debugfs_root);
}
#else
...
static inline void mpq8646_debugfs_root_create(void) {}
static inline void mpq8646_debugfs_root_destroy(void) {}
...
static const struct {
	...
	{ PB_STATUS_INPUT,       hwmon_in,   hwmon_in_alarm,   1 },	/* in1_alarm  (VIN)  */
	{ PB_STATUS_VOUT,        hwmon_in,   hwmon_in_alarm,   2 },	/* in2_alarm  (VOUT) */
...
static int __init mpq8646_init(void)
{
	int ret;

	mpq8646_debugfs_root_create();
	ret = i2c_add_driver(&mpq8646_driver);
	if (ret)
		mpq8646_debugfs_root_destroy();
	return ret;
}
module_init(mpq8646_init);

static void __exit mpq8646_exit(void)
{
	i2c_del_driver(&mpq8646_driver);
	mpq8646_debugfs_root_destroy();
}
module_exit(mpq8646_exit);
