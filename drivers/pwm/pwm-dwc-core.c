				     ...
				     const struct pwm_state *state)
{
	...
	if (dwc->features & DWC_TIM_CTRL_0N100PWM_EN) {
		...
		tmp = mul_u64_u64_div_u64(state->duty_cycle, dwc->clk_rate,
					  NSEC_PER_SEC);
		/*
		 * Calculate the width of the low and high periods in terms of
		 * input clock periods, and check whether the results are within
		 * the hardware limits of 0 to 2^32 periods.
		 */
