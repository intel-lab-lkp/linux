					      ...
					      u32 val, u8 prescale)
{
	 ...
	 * tmp = val << (info->prescale_mult * prescale) * NSEC_PER_MSEC
	 *     < 2^32 * 2^10 * 10^6
	 *     < 2^32 * 2^10 * 2^20
	 *     = 2^62
	 */
					 ...
					 void *_wfhw)
{
	...
	if (rzg2l_gpt->channel_request_count[ch] > 1) {
		...
		if (rzg2l_gpt_is_ch_enabled(rzg2l_gpt, sibling_ch, NULL)) {
			...
			wfhw->prescale = rzg2l_gpt->prescale[ch];
		}
	} else {
		wfhw->prescale = info->calculate_prescale(period_ticks);
		rzg2l_gpt->prescale[ch] = wfhw->prescale;
	}
				    ...
				    const void *_wfhw)
{
	...
	if (wfhw->gtpr) {
		bool enable = rzg2l_gpt_is_ch_enabled(rzg2l_gpt, pwm->hwpwm, NULL);
		u8 sibling_ch = rzg2l_gpt_sibling(pwm->hwpwm);
		 ...
		 */
		if (!enable || (enable && !rzg2l_gpt_is_ch_enabled(rzg2l_gpt, sibling_ch, NULL)))
			rzg2l_gpt->period_ticks[ch] =
				rzg2l_gpt_calculate_cycles(wfhw->gtpr, info->prescale_mult,
							   wfhw->prescale);
	} else {
		if (rzg2l_gpt_is_ch_enabled(rzg2l_gpt, pwm->hwpwm, NULL))
...
static int rzg2l_gpt_poeg_link_channels(struct device *dev,
					struct rzg2l_gpt_chip *rzg2l_gpt,
					struct of_phandle_args *of_args)
{
	struct device_node *np __free(device_node) = of_args->np;
	u32 poeg_grp, bitpos;

	if (of_args->args[0] >= RZG2L_MAX_HW_CHANNELS)
		return dev_err_probe(dev, -EINVAL, "Invalid channel %u >= %u\n",
	...
	if (!of_property_read_u32(of_args->np, "renesas,poeg-id", &poeg_grp)) {
		if (poeg_grp > RZG2L_LAST_POEG_GROUP)
			return dev_err_probe(dev, -EINVAL, "Invalid poeg group %u > %u\n",
			       ...
			       struct rzg2l_gpt_chip *rzg2l_gpt)
{
	...
	num_poeg_pairs = of_property_count_u32_elems(pdev->dev.of_node, poeg_name);
	if (num_poeg_pairs < 0)
		return num_poeg_pairs;

	if (num_poeg_pairs & 1)
		return -EINVAL;

	num_poeg_pairs >>= 1;
	for (unsigned int i = 0; i < num_poeg_pairs; i++) {
		...
		ret = rzg2l_gpt_poeg_link_channels(&pdev->dev, rzg2l_gpt, &of_args);
		if (ret)
			return ret;
	}
