...
static int dwc_pwm_plat_probe(struct platform_device *pdev)
{
	...
	if (ret) {
		ret = dev_err_probe(dev, ret, "failed to enable timer clock\n");
		goto disable_busclk;
	}
	...
	if (!pwm_en) {
		...
		if (ret) {
			ret = dev_err_probe(dev, ret, "failed to reset\n");
			goto disable_clk;
		}
	}
	...
	if (ret) {
		ret = dev_err_probe(dev, ret, "failed to add pwm chip\n");
		goto pm_disable;
	}
