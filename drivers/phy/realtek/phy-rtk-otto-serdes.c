...
static int rtsds_fw_load(struct rtsds_ctrl *ctrl)
{
	...
	ret = device_property_read_string(ctrl->dev, "firmware-name", &fwprop);
	if (ret)
	...
	ret = firmware_request_nowarn(&ctrl->firmware, fwname, ctrl->dev);
	if (ret < 0) {
		msg = "not found";
		goto error;
	}
