...
static int cronos_led_color_store(struct sony_cronos_smc *chip, struct sony_cronos_led *led)
{
	...
	if (ret) {
		dev_err(chip->dev, "Failed to set color value 0x%02x to LED register 0x%02x\n", byte,
			led->led_register);
		return ret;
	}
	return 0;
...
static int cronos_led_set_brightness(struct led_classdev *cdev, enum led_brightness brightness)
{
