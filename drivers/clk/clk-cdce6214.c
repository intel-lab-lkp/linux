...
static int cdce6214_get_clkout_fmt(struct cdce6214 *priv, struct device_node *np)
{
	...
	ret = of_property_read_u32(np, "ti,clkout-fmt", &fmt);
	if (ret)
		return ret;
...
static int cdce6214_get_clkin_fmt(struct cdce6214 *priv, struct device_node *np)
{
	...
	ret = of_property_read_u32(np, "ti,clkin-fmt", &fmt);
	if (ret)
		return ret;
				  ...
				  unsigned int reg)
{
	...
	ret = of_property_read_u32_array(np, "ti,cmos-mode", cmos_mode, 2);
	if (ret)
		return ret;
...
err_illegal_cmos_mode:
	dev_err(priv->dev, "%pOF: illegal cmos mode %u\n", np, cmode);
