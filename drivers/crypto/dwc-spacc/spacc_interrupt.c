...
static inline u32 _spacc_get_stat_cnt(struct spacc_device *spacc)
{
	u32 fifo;
				...
				unsigned long *lock_flag)
{
	...
	*num_popped = 0;
	...
	while ((jobs = _spacc_get_stat_cnt(spacc))) {
		while (jobs-- > 0) {
			...
			if (swid >= SPACC_MAX_JOBS) {
				dev_warn(spacc->dptr,
					"out-of-range swid %u from dropped\n",
					swid);
				continue;
			}
			if (spacc->job_lookup[swid] == SPACC_JOB_IDX_UNUSED) {
				dev_warn(spacc->dptr,
					"stale swid %u dropped\n", swid);
				continue;
			}
			...
			/*
			 * Release the swid before running the callback so a
			 * duplicate/stale STATUS entry for this swid is
			 * dropped on the next pass. A HW-completed job is
			 * dispatched here exactly once; buffered jobs that
			 * never reached HW are dispatched only by
			 * spacc_process_jb(). The two sets are disjoint,
			 * so each job's cb runs once.
			 */
	...
	spacc_process_jb(spacc, lock_flag);
