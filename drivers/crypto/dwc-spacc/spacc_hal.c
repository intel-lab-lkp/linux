			   ...
			   unsigned long nword)
{
	...
	while (nword--) {
		__raw_writel((__force u32)get_unaligned_be32(src), addr);
		src += 4;
		addr++;
	}
}
			     ...
			     unsigned long nword)
{
	...
	while (nword--) {
		put_unaligned_be32((__force u32)__raw_readl(addr), dst);
		addr++;
		dst += 4;
	}
}
				...
				unsigned long nword)
{
	...
	while (nword--) {
		put_unaligned_le32((__force u32)__raw_readl(addr), dst);
		addr++;
		dst += 4;
	}
}
...
/*
 * DDT DMA pools live per-device in struct spacc_priv (spacc_core.h),
 * not as file-scope statics. A single global pool set previously got
 * allocated against whichever device happened to probe first: every
 * other instance's DMA then ran through an IOMMU domain that wasn't
 * its own, and removing that first device destroyed the pools out
 * from under every other still-active instance. dev_get_drvdata()
 * recovers the right instance's pools from any struct device *
