					 ...
					 bool panic)
{
	...
	if (!panic) {
		...
		/*
		 * PANIC MODE: Host is already claimed via mmcpstore_stop_all_background_io()
		 * (mmc_panic_claim_host) from mmcpstore_panic_write(). Do not claim again here
		 * or mmc_panic_claim_host() logs duplicate state lines.
		 */
		if (mmc_card_mmc(cxt->card)) {
	...
	if (panic) {
		...
		/* Clean up SDHCI state after the panic request completes */
		if (host->ops->panic_complete)
			host->ops->panic_complete(host, &mrq);
...
static int mmcpstore_stop_all_background_io(struct mmcpstore_context *cxt)
{
	...
	/*
	 * Claim the host for panic I/O. This will internally call panic_prepare()
	 * to drain in-flight requests and handle runtime PM suspend state, ensuring
	 * the controller is safe for panic writes.
	 */
 ...
 *
 * Return: NOTIFY_DONE on completion.
 */
