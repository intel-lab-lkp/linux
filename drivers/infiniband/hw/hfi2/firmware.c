...
#define AUGMENT_SIZE \
	(sizeof(struct augmented_firmware_file) - sizeof(struct firmware_file))
...
static const u8 pcie_serdes_addrs[2][NUM_PCIE_SERDES] = {
	...
	{ 0x2f, 0x31, 0x33, 0x35, 0x37, 0x39, 0x3b, 0x3d, 0x3f, 0x41, 0x43,
...
static int __read_8051_data(struct hfi2_devdata *dd, u32 addr, u64 *result)
{
	...
	while ((hfi2_read_csr(dd, DC_DC8051_CFG_RAM_ACCESS_STATUS) &
		DC_DC8051_CFG_RAM_ACCESS_STATUS_ACCESS_COMPLETED_SMASK) == 0) {
		count++;
...
static int invalid_header(struct hfi2_devdata *dd, const char *what, u32 actual,
			  u32 expected)
{
	if (actual == expected)
		return 0;
			       ...
			       struct firmware_details *fdet)
{
	...
	hfi2_cdbg(FIRMWARE, "  size           0x%03x (0x%03x bytes)", css->size,
		  4 * css->size);
	...
	if (ret) {
		...
		struct firmware_file *ff =
			(struct firmware_file *)fdet->fw->data;
 ...
 */
static void write_rsa_data(struct hfi2_devdata *dd, int what, const u8 *data,
			   int nbytes)
{
			      ...
			      struct firmware_details *fdet)
{
	 ...
	 */
	reg = DC_DC8051_CFG_RST_M8051W_SMASK | DC_DC8051_CFG_RST_CRAM_SMASK |
	      DC_DC8051_CFG_RST_DRAM_SMASK | DC_DC8051_CFG_RST_IRAM_SMASK |
	 ...
	 */
	hfi2_write_csr(dd, MISC_CFG_FW_CTRL,
		     ...
		     u32 data_in)
{
	...
	hfi2_sbus_request(dd, receiver_addr, data_addr, READ_SBUS_RECEIVER,
			  data_in);
...
void hfi2_fabric_serdes_reset(struct hfi2_devdata *dd)
{
	...
	if (hfi2_is_ax(dd)) {
		...
		hfi2_sbus_request(dd, ra, 0x07, WRITE_SBUS_RECEIVER,
				  0x00000011);
...
static int check_meta_version(struct hfi2_devdata *dd, u32 *system_table)
{
	...
	if (!pcfgcache->config_tables[PLATFORM_CONFIG_SYSTEM_TABLE]
		     .table_metadata)
		return -EINVAL;

	meta_ver_meta = *(pcfgcache->config_tables[PLATFORM_CONFIG_SYSTEM_TABLE]
				  .table_metadata +
...
int hfi2_parse_platform_config(struct hfi2_pportdata *ppd)
{
	...
	while (ptr < (u32 *)(dd->platform_config.data + file_length)) {
		...
		table_length_dwords =
			(*ptr >> PLATFORM_CONFIG_HEADER_TABLE_LENGTH_SHIFT) &
			((1 << PLATFORM_CONFIG_HEADER_TABLE_LENGTH_LEN_BITS) -
			 1);

		table_type =
			(*ptr >> PLATFORM_CONFIG_HEADER_TABLE_TYPE_SHIFT) &
			((1 << PLATFORM_CONFIG_HEADER_TABLE_TYPE_LEN_BITS) - 1);

		/* Done with this set of headers */
		ptr += 2;

		if ((ptr + table_length_dwords + 1) >
		    (u32 *)(dd->platform_config.data + file_length)) {
			dd_dev_err(dd,
				   "%s: Table overruns file at offset %ld\n",
				   __func__,
				   (ptr - (u32 *)dd->platform_config.data));
			ret = -EINVAL;
			goto bail;
...
static void get_integrated_platform_config_field(
	...
	enum platform_config_table_type_encoding table_type, int field_index,
	u32 *data)
{
	...
	case PLATFORM_CONFIG_TX_PRESET_TABLE:
		...
		if (field_index == TX_PRESET_TABLE_PRECUR)
			...
			*data = (tx_preset & TX_ATTN_SMASK) >> TX_ATTN_SHIFT;
					  ...
					  u32 *field_start_bits)
{
	...
	case PLATFORM_CONFIG_VARIABLE_SETTINGS_TABLE:
		if (field && field < platform_config_table_limits[table])
			src_ptr =
				pcfgcache->config_tables[table].table_metadata +
				field;
		break;
	default:
	...
	if (field_len_bits)
		*field_len_bits =
			(*src_ptr >> METADATA_TABLE_FIELD_LEN_SHIFT) &
	...
	int field_index, u32 *data, u32 len)
{
	...
	if (!src_ptr || len < field_len_bits || !field_len_bits)
		return -EINVAL;

	src_ptr += (field_start_bits / 32);
	*data = (*src_ptr >> (field_start_bits % 32)) &
		(field_len_bits == 32 ? ~0U : (1U << field_len_bits) - 1);
...
static void dump_fw_version(struct hfi2_devdata *dd)
{
	...
	hfi2_sbus_request(dd, SBUS_MASTER_BROADCAST, 0x02, WRITE_SBUS_RECEIVER,
			  0);
	hfi2_sbus_request(dd, SBUS_MASTER_BROADCAST, 0x07, WRITE_SBUS_RECEIVER,
			  0x1);
