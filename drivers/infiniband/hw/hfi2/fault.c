...
static int _fault_stats_seq_show(struct seq_file *s, void *v)
{
	...
	for (pidx = 0; pidx < dd->num_pports; pidx++) {
		for (j = dd->rsrcs.ppr[pidx].rcv_context_base;
		     j < dd->rsrcs.ppr[pidx].first_dyn_alloc_ctxt; j++) {
	...
	seq_printf(s, "%02llx %llu/%llu (faults rx:%llu faults: tx:%llu)\n", i,
		   (unsigned long long)n_packets, (unsigned long long)n_bytes,
				   ...
				   size_t len, loff_t *pos)
{
	...
	for (ptr = data; *ptr; ptr = end + 1, token = ptr) {
		...
		if (range_start == range_end && range_start == -1UL) {
			bitmap_zero(fault->opcodes,
				    sizeof(fault->opcodes) * BITS_PER_BYTE);
			break;
		}
		/* Check the inputs */
				  ...
				  size_t len, loff_t *pos)
{
	...
	while (bit < bitsize) {
		...
		if (zero - 1 != bit)
			size += scnprintf(data + size, datalen - size - 1,
				...
				u8 direction)
{
	...
	if (ret) {
		...
		ibd->fault->skip_usec =
			jiffies + usecs_to_jiffies(ibd->fault->fault_skip_usec);
	}
	return ret;
}
