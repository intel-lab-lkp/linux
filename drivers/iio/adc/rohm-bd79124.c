				   ...
				   unsigned int ngpios)
{
	...
	*valid_mask = 0;
	...
	ret = iio_adc_device_get_channels(gc->parent, adc_channels,
...
static int bd79124_mux_init(struct bd79124_data *data)
{
	...
	num_adc = iio_adc_device_get_channels(data->dev, &adc_channels[0],
					      BD79124_MAX_NUM_CHANNELS);
	if (num_adc < 0)
		return num_adc;
...
static int bd79124_probe(struct i2c_client *i2c)
{
	...
	data->vmax = devm_regulator_get_enable_read_voltage(dev, "vdd");
	if (data->vmax < 0)
