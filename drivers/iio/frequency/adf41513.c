				 ...
				 char *buf)
{
	...
	int_part_hz = div_u64_rem(freq_uhz, MICROHZ_PER_HZ, &frac_part_uhz);
	return sysfs_emit(buf, "%llu.%06u\n", int_part_hz, frac_part_uhz);
}
...
{
	struct adf41513_state *st = iio_priv(indio_dev);
	s64 int_part_hz, frac_part_uhz;
	u64 freq_uhz;
	int ret;

	ret = iio_str_to_fixpoint64(buf, MICROHZ_PER_HZ / 10, &int_part_hz,
				    &frac_part_uhz);
	if (ret)
		return ret;

	freq_uhz = int_part_hz * MICROHZ_PER_HZ + frac_part_uhz;
	guard(mutex)(&st->lock);
