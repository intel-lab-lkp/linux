...
def _member_type_names(value) -> list:
    ...
    entry per member so a type used twice is counted twice."""
    if isinstance(value, _XdrStruct):
        fields = value.fields
    elif isinstance(value, _XdrPointer):
    ...
    for definition in root.definitions:
        ...
        for marked in sorted(aggregate_members):
            ...
            if isinstance(field, _XdrVariableLengthArray):
                ...
                # The emitted optional-data decoder is a placeholder
                # that consumes nothing, so an argument type carrying
                # one would decode its later members from the wrong
                # stream position.
