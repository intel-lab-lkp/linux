...
check_attr_mode()
{
	...
	mode=$(stat -c %a "${file}") || fail "failed to stat ${file}"
	[ "${mode}" = "${expected}" ] || \
		fail "cpu${current_cpu}/${attr} mode expected ${expected}, got ${mode}"
}
...
check_sysfs_layout()
{
	...
	check_attr_mode affected_cpus 444

	for attr in "${COMMON_RW_ATTRS[@]}"; do
		check_attr_mode "${attr}" 600
