		   ...
		   struct buffer_head *bh)
{
	...
	int c1, c2 = 0;
	go_down:
	if (1) if (hpfs_stop_cycles(s, a, &c1, &c2, "hpfs_bplus_lookup")) return -1;
	...
	for (i = 0; i < btree->n_used_nodes; i++)
		if (le32_to_cpu(btree->u.external[i].file_secno) <= sec &&
			...
			if (1) if (hpfs_chk_sectors(s, a, 1, "data")) {
				brelse(bh);
				return -1;
			}
			if (inode) {
	    ...
	    unsigned len, char *buf)
{
	...
	while (len) {
		...
		if (1) if (hpfs_chk_sectors(s, sec, 1, "ea #1")) return -1;
	     ...
	     unsigned len, const char *buf)
{
	...
	while (len) {
		...
		if (1) if (hpfs_chk_sectors(s, sec, 1, "ea #2")) return -1;
		if (!(data = hpfs_map_sector(s, sec, &bh, (len - 1) >> 9)))
			return -1;
		l = 0x200 - (pos & 0x1ff); if (l > len) l = len;
