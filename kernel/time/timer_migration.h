 ...
 *			in CPU hotplug path as the CPU is able to go idle before
 *			the timer migration hierarchy hotplug AP is reached.
 *			During this phase, the CPU has to handle the global
 *			timers on its own and must not act as a migrator.
