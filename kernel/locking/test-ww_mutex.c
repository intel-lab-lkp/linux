...
static int run_tests(struct ww_class *class)
{
	 ...
	 * all kernel threads to be run-able. With N CPUs and N + 1
	 * worker threads, deadlock may happen. Hence, adjust
	 * min_active. Set both max and min, in case max is too
	 * small. Cap N so that N + 1 doesn't exceed WQ_MAX_ACTIVE.
	 */
